...
impl<const MTU: usize, const MAX_PIECES: usize, const MAX_TRANSFERS: usize> Assembler<MTU, MAX_PIECES, MAX_TRANSFERS> where [(); MTU - 1]: Sized {
    ...
    pub fn eat(&mut self, can_id: CanId, bytes: &[u8]) {
        match self.find_transfer_by_can_id(can_id) {
            Some(transfer) => {
                ...
                *transfer = match *transfer {
                    ...
                    Transfer::Assembling(_, transfer_assembling) => {
                        // New chunk of data came, find a place for it
                        self.handle_assembling(can_id, bytes, transfer_assembling)
    ...
    fn handle_assembling(&mut self, can_id: CanId, bytes: &[u8], transfer_assembling: TransferAssembling) -> Transfer {
        loop {
            match self.find_empty_slot() {
                Some(i) => {
                    ...
                    return Transfer::Assembling(can_id, transfer_assembling);
                }
                None => {
                    // no empty slots, destroy some lower priority transfer and free up
                    loop {
                        match self.find_lower_priority_transfer(can_id.priority) {
                            Some(lp_transfer) => {
                                match lp_transfer {
                                    Transfer::Empty => {
                                        unreachable!()
                                    ...
                                    Transfer::Done(_, td) => {
                                        ...
                                        self.counters.destroyed_other_done += 1;
                                        break;
                                    }
                                    Transfer::Error(_) => {
                            ...
                            None => {
                                ...
                                self.remove_pieces(transfer_assembling.first_piece);
    ...
    fn get_transfer(&self, idx: TransferIdx) -> &mut Transfer {
        unsafe { &mut *(self.transfers.as_ptr().offset(idx as isize) as * const _ as * mut Transfer) }
    }

    fn find_transfer_by_can_id(&self, id: CanId) -> Option<&mut Transfer> {
        use Transfer::*;
        for (i, t) in self.transfers.iter().enumerate() {
            match t {
                ...
                Assembling(idu, _) | Done(idu, _) | Error(idu) => {
                    if id == *idu {
                        return Some(self.get_transfer(i as TransferIdx))
                    }
                }
            }
        }
